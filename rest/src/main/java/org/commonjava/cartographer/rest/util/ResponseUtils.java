...
import org.commonjava.cartographer.rest.dto.CreationDTO;
...
import org.commonjava.propulsor.deploy.undertow.util.HttpUtils;
import org.commonjava.propulsor.deploy.undertow.util.StandardApplicationContent;
...
public final class ResponseUtils
{
    ...
    public static Response formatCreatedResponse( final String baseUri,
                                                  final String... params )
            throws URISyntaxException
    {
                                                                ...
                                                                final ObjectMapper objectMapper )
    {
        ...
        try
        {
            response = Response.created( location )
                               ...
                               .type( StandardApplicationContent.application_json )
                               .build();
        }
    ...
    {
        return formatOkResponseWithEntity( json, StandardApplicationContent.application_json );
    }
    ...
    public static Response formatOkResponseWithJsonEntity( final Object dto, final ObjectMapper objectMapper )
    {
        ...
        {
            response =
                    Response.ok( objectMapper.writeValueAsString( dto ), StandardApplicationContent.application_json ).build();
        }
        catch ( final JsonProcessingException e )
        {
            response = formatResponse( e, "Failed to serialize DTO to JSON: " + dto );
                                               ...
                                               final String message, final boolean throwIt )
    {
        ...
        else if ( ( error instanceof CartoRESTException ) && ( (CartoRESTException) error ).getCode() > 0 )
        {
            final int sc = ( (CartoRESTException) error ).getCode();
        ...
        Response response = Response.status( code )
                       .header( ApplicationHeader.content_type.key(), StandardApplicationContent.text_plain )
                       .entity( msg )
                       .build();
